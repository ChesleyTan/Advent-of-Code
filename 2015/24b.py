...
def calculate_qe(packages):
    ...
    for p in packages:
        prod *= p
    return prod
def partition_groups(packages, g1=[], g1sum=0, g2=[], g2sum=0, g3=[], g3sum=0, g4=[], g4sum=0):
    ...
    if len(g1) > min_passenger_packages or (len(g1) == min_passenger_packages and calculate_qe(g1) >= min_passenger_packages_qe):
        if len(g2) > min_passenger_packages or (len(g2) == min_passenger_packages and calculate_qe(g2) >= min_passenger_packages_qe):
            if len(g3) > min_passenger_packages or (len(g3) == min_passenger_packages and calculate_qe(g3) >= min_passenger_packages_qe):
                if len(g4) > min_passenger_packages or (len(g4) == min_passenger_packages and calculate_qe(g4) >= min_passenger_packages_qe):
                    return
    if g1sum > balance_weight or g2sum > balance_weight or g3sum > balance_weight or g4sum > balance_weight:
        return
    if g1sum == g2sum == g3sum == g4sum == balance_weight:
        ...
        if len(g4) < min_passenger_packages:
            min_passenger_packages = len(g4)
            min_passenger_packages_qe = calculate_qe(g4)
        elif len(g4) == min_passenger_packages:
            min_passenger_packages_qe = min(min_passenger_packages_qe, calculate_qe(g4))
        print g1, g2, g3, g4, 'QE: ' + str(min_passenger_packages_qe)
    else:
        ...
        partition_groups(new_packages, g1=new_g1, g1sum=g1sum+p, g2=g2,
                g2sum=g2sum, g3=g3, g3sum=g3sum, g4=g4, g4sum=g4sum)
        new_g2.append(p)
        partition_groups(new_packages, g1=g1, g1sum=g1sum, g2=new_g2,
                g2sum=g2sum+p, g3=g3, g3sum=g3sum, g4=g4, g4sum=g4sum)
        new_g3.append(p)
        partition_groups(new_packages, g1=g1, g1sum=g1sum, g2=g2,
                g2sum=g2sum, g3=new_g3, g3sum=g3sum+p, g4=g4, g4sum=g4sum)
        new_g4.append(p)
        partition_groups(new_packages, g1=g1, g1sum=g1sum, g2=g2,
                g2sum=g2sum, g3=g3, g3sum=g3sum, g4=new_g4, g4sum=g4sum+p)
