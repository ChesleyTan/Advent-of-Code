...
nextPose :: Grid -> Pose -> String -> String
nextPose _ pose letters | trace (show pose ++ " " ++ show letters) False = undefined
nextPose grid pose letters =
    let nextCoord = case pose of
            Pose (x, y) DUp -> (x, y - 1)
            Pose (x, y) DDown -> (x, y + 1)
            Pose (x, y) DLeft -> (x - 1, y)
            Pose (x, y) DRight -> (x + 1, y)
        currentCoord = getCoord pose
        currentLetter = grid !! snd currentCoord !! fst currentCoord
        newLetters = if isLetter currentLetter
                        then letters ++ [currentLetter]
                        else letters
    in if inBounds grid nextCoord
        then trace "inBounds" $ nextPose grid (Pose nextCoord (getDir pose)) newLetters
        else
            let nextPos = getLRNeighbors grid pose |> take 1
            in if null nextPos
                then newLetters
                else trace (show nextCoord) $ nextPose grid (head nextPos) newLetters

main :: IO ()
main = do
    input <- map T.unpack . T.lines <$> T.readFile "19a.input"
    let startingPose = Pose (13, 0) DDown
    print $ nextPose input startingPose ""
