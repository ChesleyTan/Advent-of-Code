...
if __name__ == "__main__":
    with open("25.input", 'r') as f:
        ...
        for a in xrange(0, 10000):
            ...
            ip = 0
            signals = []
            while ip < len(lines) and len(signals) < 100:
                try:
                    cmd = lines[ip]
                    tokens = cmd.split()
                    if tokens[0] == "cpy":
                                        ...
                                        if cpy_tokens[1] in regs and cpy_tokens[2] in regs:
                                            ...
                                            if "dec " + reg2 in last5_ins:
                                                ...
                                                if inc_ins:
                                                    ...
                                                    ip += 1
                                                    continue
                                        if cpy_tokens[1] not in regs and cpy_tokens[2] in regs:
                                            reg2 = cpy_tokens[2]
                                            if "dec " + reg2 in last5_ins:
                                                dec_reg = reg2
                                                inc_ins = [ins for ins in last5_ins if ins.split()[0] == "inc"]
                                                if inc_ins:
                                                    ...
                                                    inc_ins = inc_ins[0]
                                                    inc_reg = inc_ins.split()[1]
                                                    regs[inc_reg] += int(cpy_tokens[1]) * regs[tokens[1]]
                                                    regs[reg2] = 0
                                                    regs[tokens[1]] = 0
                                                    ip += 1
                                                    continue
                        ...
                        if tokens[1] in regs:
                            ...
                            value = int(tokens[1])
                        if len(signals) > 0 and signals[-1] == value:
                            #print "FAIL"
                            break
