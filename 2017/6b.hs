...
import Data.Map.Strict as M (fromList, empty, size, findWithDefault, adjust, insert)
...
run banks memo steps =
    if M.findWithDefault (-1) banks memo /= -1
    then steps - M.findWithDefault (-1) banks memo
    else run (redistribute banks) (M.insert banks steps memo) (steps + 1)

main :: IO String
main = do
    input <- map ((read :: String -> Int) . T.unpack) . T.words . T.strip <$> T.readFile "6a.input"
    print input
    print $ run input M.empty 0
    return ""
